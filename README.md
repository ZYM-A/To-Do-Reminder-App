...
- [v0.3.0 安装包](https://github.com/ZYM-A/To-Do-Reminder-App/releases/download/v0.3.0/richang-v0.3.0-debug.apk)
- [更新记录](CHANGELOG.md)
...
底部选择 **纪念日 → 添加纪念日**，填写名称、历法、日期和备注：

- **公历 / 农历自由选择**：切换历法时保留同一个实际日期；点击日期可重新选择。
- 农历提供独立年、月、日选择器，支持农历 1900—2100 年，只列出实际存在的月份（包括闰月）和日期，并显示对应公历日期。
- 未来日期显示“还有 N 天”，过去日期显示“已经 N 天”，当天显示“就是今天”；按手机当前日期计算自然日差，今天为 0 天、明天为 1 天。
- 开启“每年重复”后按所选历法倒数下一次纪念日，原始日期仍会保留。公历 2 月 29 日在平年按 2 月 28 日纪念，闰年恢复。
- 农历闰月纪念日遇到没有同名闰月的年份，按同名普通月纪念；原日期为三十而当月只有廿九时，按廿九纪念。以后遇到对应闰月或三十会恢复原日期规则。
- 切换农历年份或月份时也使用上述日期回退规则，确认前可查看对应公历。
- 纪念日只记录日期，不发送到点通知；点击卡片可修改或删除，数据保存在本机。
- 数据库升级到 v3，已有纪念日默认保持公历，保留原待办、日期、备注和重复设置。
- v0.3.0 沿用原测试签名，可以覆盖安装 v0.1.0 / v0.2.0，无需卸载。
...
当前版本的检查结果见 [v0.3.0 验证记录](docs/VALIDATION-v0.3.0.md)；[第一版验证记录](docs/VALIDATION.md) 另行保留。
