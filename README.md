# 日常 · 安卓日程待办

一个使用 Kotlin 和 Jetpack Compose 开发的中文安卓应用。面向个人使用，日程、纪念日和日记保存在手机本地；AI 伴侣可选接入自己的模型服务商账户。最低支持 Android 8.0（API 26）。

## 下载测试版

- [版本发布与 APK 下载](https://github.com/ZYM-A/To-Do-Reminder-App/releases)
- [v0.5.1 安装包](https://github.com/ZYM-A/To-Do-Reminder-App/releases/download/v0.5.1/richang-v0.5.1-debug.apk)
- [更新记录](CHANGELOG.md)

打开 Releases 中对应版本，在 Assets 中下载 `.apk` 文件。源码下载包不包含安装包；编译工具、缓存、本机配置和签名密钥也不提交到仓库。

## 获取源码与版本管理

```bash
git clone https://github.com/ZYM-A/To-Do-Reminder-App.git
cd To-Do-Reminder-App
```

`main` 保存开发源码，`v0.1.0` 等标签标记发布版本。后续修改可提交并推送：

```bash
git add .
git commit -m "Describe the change"
git push
```

后续发布请先更新应用版本号和更新记录，完成构建与检查，再创建版本标签，将 APK 上传到对应 Release。当前是开发测试签名；持续覆盖安装需要保持签名密钥一致，密钥应安全保存在本机，不上传公开仓库。


## AI 伴侣（v0.5.0）

在任一主栏目首页右上角点爱心按钮，进入独立的伴侣聊天页面。底部仍为 **日程、纪念日、日记本** 三项。

- **人设编辑**：设置伴侣名字、对你的称呼、性格和说话风格；默认温柔陪伴，保存到本机。
- **个人模型接入**：填写自己的 HTTPS 基础地址、API 密钥、模型 ID；提供 DeepSeek、硅基流动地址预设和自定义地址。支持兼容 Chat Completions 的文字接口，不内置密钥或收费账户。
- **测试与模型选择**：确认接收地址后获取模型列表，也可手动填写模型 ID。保存配置本身不联网；获取列表成功不等于模型一定支持聊天。
- **文字聊天**：发送消息、等待回复、保留本地记录；失败后手动重试，不自动重试。回复会注明使用的模型。
- **话题管理**：新建、切换和删除话题；每次请求只取当前话题的有限上下文，其他话题不参与。
- **聊天统计**：成功消息数、AI 回复数、活跃天数、相识天数，以及服务商返回的 token 用量。用量可能不完整，不据此估算费用，实际费用以服务商账单为准。

首次使用：**爱心 → 右上角菜单 → 模型设置 → 选择服务商或填基础地址 → 填密钥及模型 ID → 保存配置 → 返回聊天**。发送前会说明接收方与数据范围；修改人设、模型配置或切换话题后重新确认。

### 数据与隐私

接口配置和密钥由 Android Keystore 提供密钥，通过 AES-GCM 加密保存在本机；密钥输入页遮挡密钥并禁止截图，未保存的密钥不写入界面恢复状态。不要把密钥发到公开仓库、Issue 或聊天截图里。

发送聊天时，API 密钥会用于向你选择的服务商鉴权；服务商还会收到当前消息、人设及当前话题最多最近 20 条成功消息（历史正文最多 24000 字）。服务商可能保存请求并收费。当前实现仅接受 HTTPS，不跟随重定向，不输出服务商错误正文。

人设及聊天正文保存在应用私有目录的本地 SQLite 数据库，目前不额外加密正文；不会自动读取或发送日记、日程、纪念日或其他话题，没有遥测、自动上传或云同步。系统备份禁用。卸载、清除应用数据会删除记录和配置；删除本机话题不会删除服务商持有的数据。输入草稿仅保存在运行中的内存里，进程结束后可能丢失。

当前只实现单个人设和基础文字聊天，不含长期记忆、图片、语音通话或主动消息。长对话只携带有限上下文，AI 可能忘记更早的内容。涉及额外私人数据读取或外发的后续功能需另外确定范围。

## 三栏导航与日记本（v0.4.0）

底部导航只有 **日程、纪念日、日记本** 三项：

- **日程**：在页面内切换“今日 / 日历 / 全部”；“全部”中可筛选待完成和已完成。原有待办编辑、提前提醒和每日 / 每周重复功能保留。
- **纪念日**：公历 / 农历选择及倒数功能保持不变。
- **日记本**：点“写日记”选择日期、填写正文，标题可选；同一天可保存多篇日记，也可补写过去的日期。
- 日记按日期从近到远排列，同日按最近修改排列。支持标题和正文关键词搜索，点卡片可阅读、编辑或删除。
- 日记标题最多 100 字、正文最多 50000 字；保留段落和换行。点击“保存日记”写入本地，返回时有未保存内容会提示是否放弃。
- 日记保存在手机本地，不联网、不云同步；卸载或清除应用数据会删除记录，目前没有导出功能。
- 数据库升级到 v4 时仅添加日记表，保留原待办和公农历纪念日。v0.4.0 使用原测试签名，可直接覆盖安装，无需卸载。

## 纪念日倒数（v0.3.0 起）

底部选择 **纪念日 → 添加纪念日**，填写名称、历法、日期和备注：

- **公历 / 农历自由选择**：切换历法时保留同一个实际日期；点击日期可重新选择。
- 农历提供独立年、月、日选择器，支持农历 1900—2100 年，只列出实际存在的月份（包括闰月）和日期，并显示对应公历日期。
- 未来日期显示“还有 N 天”，过去日期显示“已经 N 天”，当天显示“就是今天”；按手机当前日期计算自然日差，今天为 0 天、明天为 1 天。
- 开启“每年重复”后按所选历法倒数下一次纪念日，原始日期仍会保留。公历 2 月 29 日在平年按 2 月 28 日纪念，闰年恢复。
- 农历闰月纪念日遇到没有同名闰月的年份，按同名普通月纪念；原日期为三十而当月只有廿九时，按廿九纪念。以后遇到对应闰月或三十会恢复原日期规则。
- 切换农历年份或月份时也使用上述日期回退规则，确认前可查看对应公历。
- 纪念日只记录日期，不发送到点通知；点击卡片可修改或删除，数据保存在本机。
- 数据库升级到 v3，已有纪念日默认保持公历，保留原待办、日期、备注和重复设置。
- v0.3.0 沿用原测试签名，可以覆盖安装 v0.1.0 / v0.2.0，无需卸载。

## 已实现

- 今日待办与逾期提示；按月份、日期查看日程；全部任务和已完成筛选。
- 新增、编辑、删除任务；标题、备注、日期、时间；完成和恢复一次性任务。
- 准时或提前 5 / 10 / 30 / 60 分钟提醒，也可以关闭单个任务提醒。
- 每天、每周重复。即使没有勾选完成，也会继续按固定日期提醒；勾选完成将任务日期推进到下一次。
- SQLite 本地持久化；数据库操作放在后台线程。
- 系统通知权限和准时提醒权限入口；修改、完成、删除任务时取消对应旧提醒。
- 设备重启并解锁后、应用更新后、系统时间改变后重新安排提醒。
- 通知点击打开对应任务；浅色和深色主题。

## 打开项目

在 Android Studio 中选择 **Open**，打开本目录，等待 Gradle Sync。

构建配置：JDK 17、Android SDK 35、Build Tools 35.0.0、Gradle 8.11.1、AGP 8.9.2、Kotlin 2.1.20。依赖版本已固定，Gradle Wrapper 附带分发包 SHA-256 校验。

`local.properties` 指向本机 SDK，不提交到版本控制。本次工作使用项目内 `.tools/android-sdk`；换电脑时请由 Android Studio 自动生成自己的 SDK 路径。

## 命令行构建

Windows PowerShell：

```powershell
.\scripts\build.ps1
```

脚本优先使用本次已准备好的 `.tools/gradle-8.11.1`，否则使用 Gradle Wrapper。首次运行需要联网下载依赖。构建完成后输出：

```text
app/build/outputs/apk/debug/app-debug.apk
```

也可以在已配置 SDK 和 JDK 的环境中运行：

```powershell
.\gradlew.bat :app:assembleDebug :app:testDebugUnitTest :app:lintDebug
```

如果官方 Gradle 下载重定向不可访问，可使用可访问的镜像下载相同版本，然后核对官方校验值，解压到 `.tools`。不要禁用 TLS 证书校验或忽略校验值。

## 安装和试用

将 debug APK 传到安卓手机，打开文件安装；手机可能要求为传输文件的应用允许安装来源。首次启动后，点击提示条开启通知及准时提醒权限。

快速验收：

1. 添加一个 2 分钟后的任务，设置“准时”，锁屏等待通知。
2. 点通知，确认打开的是对应任务。
3. 修改另一任务的提醒时间，确认旧时间不再提醒。
4. 删除任务，确认不会再收到该任务通知。
5. 退出并重新打开应用，确认任务仍在。
6. 创建每天重复的任务，勾选完成，确认任务移到下一天。
7. 有未来任务时重启并解锁手机，检查提醒恢复。

Debug APK 用于本地试用。发布应用商店前需要配置正式签名、隐私说明及发布版本检查。

## 第一版的规则与边界

- 日历展示任务当前的待办日期，暂不把重复任务的所有未来日期展开，也不保存每次重复完成的历史记录。
- 提醒时间已错过时：一次性任务尽快补发一次；重复任务只补发最近一次，不连续弹出历史通知。
- 未允许准时提醒时，使用系统非精确提醒，界面明确提示可能延迟；未允许通知时不会弹通知，重新授权并回到应用后恢复调度。
- 强行停止应用后，需要重新打开以恢复提醒；关机期间无法提醒。系统省电策略和通知设置仍会影响通知展示。
- 时间依据手机系统时区。跨时区后重新计算后续调度，第一版不提供独立时区设置。
- 日程、纪念日和日记保存在本机；v0.5.0 为可选 AI 伴侣增加联网权限，只有主动测试接口或确认发送聊天才发起模型请求。禁用系统云备份；卸载或清除数据会删除记录，目前尚无导出功能。
- 本次不做手机品牌专属适配。

## 代码位置

```text
app/src/main/java/com/richang/todo/
  MainActivity.kt                  生命周期、系统设置和权限入口
  TodoViewModel.kt                 界面状态与异步操作
  TodoApplication.kt               应用级数据仓库
  data/Task.kt                     任务模型与可独立测试的日期规则
  data/TaskDatabase.kt             SQLite 持久化
  data/TaskStore.kt                串行化数据更新和提醒调度
  reminders/ReminderScheduler.kt   系统闹钟与通知
  reminders/ReminderReceiver.kt    提醒触发与重启恢复
  companion/                      人设、模型接口、加密配置、话题与聊天统计
  ui/                             Compose 界面与主题
```

## 测试

- `testDebugUnitTest`：在电脑上运行重复规则、跨日期、夏令时和重复通知防护等单元测试。
- `lintDebug`：检查安卓 API 使用、资源和代码问题。
- `connectedDebugAndroidTest`：需要连接测试手机或启动安卓模拟器；包含数据库重开后保存、更新、删除验证，使用测试包的数据库，不读取用户任务。

当前版本的检查结果见 [v0.5.1 验证记录](docs/VALIDATION-v0.5.1.md)；[第一版验证记录](docs/VALIDATION.md) 另行保留。

## 官方参考

- [AGP 8.9 的 JDK、Gradle 和 SDK 兼容性](https://developer.android.com/build/releases/agp-8-9-0-release-notes)
- [安卓系统提醒调度](https://developer.android.com/develop/background-work/services/alarms)
- [通知运行时权限](https://developer.android.com/develop/ui/compose/notifications/notification-permission)
